...
def main() -> None:
    ...
    # 4. Curve fitting
    try:
        popt, pcov = curve_fit(preferential_attachment_func, k_vals, Pi_k_vals, p0=[1, 1], maxfev=10000)
    ...
    plt.loglog(k_vals, fit_y, 'r--', label=f'Fit: $\\Pi(k) \propto k^{{{alpha_fit:.2f}}}$')

    plt.xlabel('Degree $k$ at $T_0$')
    plt.ylabel('Attachment Probability $\Pi(k)$')
    plt.title(f'Evolution of Argentine Labor Market: {snakemake.wildcards.get("class_", "Unknown")}')
    plt.legend()
