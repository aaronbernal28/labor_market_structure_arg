...
def main() -> None:
	...
	if use_weighted:
		...
		# Get sets of nodes for each community
		c1_nodes = set(filtered_nodes[filtered_nodes[community_col] == c1][id_col].astype(int))
		c2_nodes = set(filtered_nodes[filtered_nodes[community_col] == c2][id_col].astype(int))
		
		# Compute inter-community betweenness centrality (subset betweenness)
		# Note: normalized=False means we get unnormalized path counts.
		bc_raw = nx.betweenness_centrality_subset(
			cost_graph, sources=list(c1_nodes), targets=list(c2_nodes), weight="cost", normalized=False
		)
		
		# By default, nx.betweenness_centrality_subset with normalized=False divides by 2 when G is undirected.
		# To get the raw sum of paths in both directions (s->t and t->s), we scale it by 2.
		betweenness = {}
		n1 = len(c1_nodes)
		n2 = len(c2_nodes)
		N = len(cost_graph)
		
		normalization = snakemake.config.get("inter_betweenness_normalization", "custom")
		
		for v in cost_graph.nodes():
			val = 2.0 * bc_raw[v]
			if normalization == "custom":
				if v in c1_nodes:
					denom = (n1 - 1) * n2 if n1 > 1 else 1.0
				elif v in c2_nodes:
					denom = n1 * (n2 - 1) if n2 > 1 else 1.0
				else:
					denom = n1 * n2
				betweenness[v] = val / denom
			elif normalization == "standard":
				# Standard subset normalization divides by (N-1)(N-2)
				betweenness[v] = val / ((N - 1) * (N - 2)) if N > 2 else 0.0
			else:
				betweenness[v] = val
	else:
	...
	log_lines: list[str] = []
	log_lines.append("=" * 60)
	log_lines.append("INTER-COMMUNITY PUBLIC POLICY BY COMMUNITIES")
	log_lines.append("=" * 60)
