...
def main() -> None:
	...
	if dataset == "enes_2021":
		...
		df_enes = df_enes.rename(columns={
			snakemake.config["datasets"][dataset]["caes_id"]: snakemake.config["datasets"]["enes_2019"]["caes_id"],
			snakemake.config["datasets"][dataset]["ciuo_id"]: snakemake.config["datasets"]["enes_2019"]["ciuo_id"],
			**{feature: FEATURES[i] for i, feature in enumerate(FEATURES) if feature in df_enes.columns}
		})
